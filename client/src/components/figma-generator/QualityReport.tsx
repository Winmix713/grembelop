...
export function QualityReport({ component }: QualityReportProps) {
  if (!component) {
    return (
      <div className="p-6 bg-gray-50 border border-red-300 rounded-md text-red-700">
        Nincs elérhető komponens adat a minőségi jelentéshez.
      </div>
    );
  }
  ...
  if (!accessibility || !responsive || !metadata) {
    return (
      <div className="p-6 bg-gray-50 border border-yellow-300 rounded-md text-yellow-700">
        A komponens adatainak egy része hiányzik, nem lehet megjeleníteni a minőségi jelentést.
      </div>
    );
  }
